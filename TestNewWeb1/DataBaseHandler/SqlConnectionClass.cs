...
namespace TestNewWeb1
{
    public class SqlConnectionClass
    {
        ...
        public DataTable SelectAll(string tableName, string orderBy = "", string sortDirection = "ASC")
        {
            try
            {
        ...
        public DataTable SelectColumns(string tableName, string[] cols, string orderBy = "", string sortDirection = "ASC")
        {
            try
            {
        ...
        public DataTable SelectAllCondition(string tableName, string condition, string orderBy = "", string sortDirection = "ASC")
        {
            try
            {
        ...
        public DataTable SelectColumnsCondition(string tableName, string[] cols, string condition, string orderBy = "", string sortDirection = "ASC")
        {
            try
            {
                if (cols.Length == 0) return null;
        ...
        public DataTable JoinTables(string[] tables, Dictionary<string, string> pkFkRelations, string condition = "", string orderBy = "", string sortDirection = "ASC")
        {
            try
            {
        ...
        public DataTable JoinTablesColumns(string[] tables, Dictionary<string, string> pkFkRelations, string[] columns, string orderBy = "", string sortDirection = "ASC")
        {
            try
            {
                if (tables.Length < 2 || pkFkRelations.Count == 0)
        ...
        public double GetSum(string tableName, string columnName, string condition = "")
        {
            try
            {
                // Build the query with ISNULL or COALESCE to handle NULL results
                string query = $"SELECT ISNULL(SUM({columnName}), 0) FROM {tableName}";
                if (!string.IsNullOrEmpty(condition))
                {
                    query += $" WHERE {condition}";
                }
            ...
            {
                throw new Exception($"Error while getting sum of column '{columnName}' from table '{tableName}': {e.Message}", e);
            }
        }

        public double GetAvg(string tableName, string columnName, string condition = "")
        {
            try
            {
                // Build the query with ISNULL or COALESCE to handle NULL results
                string query = $"SELECT ISNULL(AVG({columnName}), 0) FROM {tableName}";
                if (!string.IsNullOrEmpty(condition))
                {
                    query += $" WHERE {condition}";
                }
            ...
            {
                throw new Exception($"Error while getting average of column '{columnName}' from table '{tableName}': {e.Message}", e);
            }
        }


        public DataTable SelectTopN(string tableName, int topN, string categoryColumn)
        {
            try
            {
                // Query to get the top N rows from each category
                string query = $@"
                    WITH RankedData AS (
                        SELECT *, 
                               ROW_NUMBER() OVER (PARTITION BY {categoryColumn} ORDER BY (SELECT NULL)) AS RowNum
                        FROM {tableName}
                    )
                    SELECT * 
                    FROM RankedData
            ...
            {
                throw new Exception($"Error while selecting top {topN} rows from each category in table '{tableName}': {e.Message}", e);
            }
        }

        public DataTable SelectTopNWithCondition(string tableName, string columnName, object value, int topN)
        {
            DataTable dataTable = new DataTable();
            string query = $"SELECT TOP {topN} * FROM {tableName} WHERE {columnName} = @Value";
        ...
        public DataTable SelectTopNTable(string tableName, int topN, string orderBy = "", string sortDirection = "ASC")
        {
            try
            {
                // Specify the columns to select (use * to select all columns)
                string query = $@"
                    SELECT TOP {topN} *
                    FROM {tableName} ";
            ...
            {
                throw new Exception($"Error while selecting top {topN} rows from table '{tableName}': {e.Message}", e);
            }
        }


        public DataTable UsersWithTheirAmount()
        {
            string query = @"
                SELECT 
                    u.id AS user_id,
